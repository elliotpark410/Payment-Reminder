...
export async function handleAddReset(request: Request, response: Response, next: NextFunction) {
  try {
    ...
    const [selectResults] = await promisePool.execute<RowDataPacket[]>(selectQuery, [resetId]);
